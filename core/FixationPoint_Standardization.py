...
from head_compensation import return_s,eye_center_length
...
def caculateCoeficiente():
    ...
    X = []
    for x in ECCG_list:
        X.append(x[0] * DCF[ECCG_list.index(x)])
    Y = []
    for x in ECCG_list:
        Y.append(x[1] * DCF[ECCG_list.index(x)])
    Z_screenX = []
    for x in point_list:
        Z_screenX.append(x[0] * DCF[point_list.index(x)])
    Z_screenX = Z_screenX * 3
    Z_screenY = []
    for x in point_list:
        Z_screenY.append(x[1] * DCF[point_list.index(x)])
    Z_screenY = Z_screenY * 3
...
def shot(video_capture, frame_WIN, screen_width, screen_height, iscenter):
    ...
    while i < (frame_num - 1) * frame_interval + 1:
        if i % frame_interval == 0:
            ...
            if iscenter:
                cv2.imwrite(r"D:\\project\\EyeTracking_NN-master\\image\\frame1\\" + str(i) + ".jpg", f)
            else:
                cv2.imwrite(r"D:\\project\\EyeTracking_NN-master\\image\\frame2\\" + str(i) + ".jpg", f)
    ...
    #for i in range(j):
     #   if x == -1:
      #      frame1_path = "D:\\yandong\\EyeTracking_NN\\image\\head\\" + str(i) + ".jpg"
       #     frame1 = cv2.imread(frame1_path)
        #      x = i
        #if y == -1 and x != -1:
         #   frame2_path = "D:\\yandong\\EyeTracking_NN\\image\\head\\" + str(i) + ".jpg"
            #frame2 = cv2.imread(frame2_path)
           # if eye_center_length(frame2) != 0 and i != x:
                #s = return_s(frame1, frame2)
               # break
    if iscenter:
        for i in range(j):
            frame1_path = "D:\\project\\EyeTracking_NN-master\\image\\frame1\\" + str(i) + ".jpg"
            frame1 = cv2.imread(frame1_path)
            if eye_center_length(frame1) != 0 and x == -1:
                break

    else:
        for i in range(j):
            frame2_path = "D:\\project\\EyeTracking_NN-master\\image\\frame2\\" + str(i) + ".jpg"
            frame2 = cv2.imread(frame2_path)
            if eye_center_length(frame2) != 0 and y == -1:
                s = return_s(frame1, frame2)
                break
