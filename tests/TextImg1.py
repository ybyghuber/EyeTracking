...
#images = glob.glob('../image/gray_eye1/' + os.sep + '**.' + 'bmp')
for frame in images:
    img = cv2.imread(frame)
    img = cv2.resize(img, (0, 0), fx=5, fy=5, interpolation=cv2.INTER_AREA)
    histogram_eye, cdf = CalibrationHelper.histeq(np.array(img))
    histogram_eye = np.uint8(histogram_eye)
    img = cv2.bilateralFilter(histogram_eye, 10, 15, 15)
    f_img = find_Ellipse(img)
    ...
    #contruct = C_R(3, result, con_kernel)  # 闭操作
    ...
    #         cv2.circle(contruct, (i[0], i[1]), i[2], (0, 255, 0), 2)
    #         cv2.circle(contruct, (i[0], i[1]), 2, (0, 0, 255), 3)
        #print('circleNum= ', len(circles))

    imgs = np.hstack([img,f_img, img_thres])
    #
